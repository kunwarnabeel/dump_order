...
class Rlm_model extends CI_Model  {
  ...
  public function import_yazaki_data($customer_num,$customer_name,$file){
        set_time_limit(0);
            ...
            foreach($lines as $lineNo => $line){
                ...
                elseif($isReleaseHeader){ // read inside section data
                    ...
                    if($flag){
                        ...
                        $plantNum = $temp[1].'_'.$temp[7];
                        $rlm_key = $customer_num.'-'.$temp[1].'_'.$temp[7].'-'.$temp[8].'-'.$import_date;
    ...
    public function import_data($customer_num,$customer_name,$file){
        set_time_limit(0);
        ini_set('memory_limit', '20000M');
        $period_arr = [];
        $transaction_level_data = [];
            ...
            foreach($transaction_predefined_headers as $key => $row)
            {
                ...
                elseif($key == 'date_format'){
                    ...
                    for($i=$week;$week<10;$i++){
                        $nextMonday = date($row, strtotime($i.' monday'));
                        // if($i==0)
                        //     continue;
